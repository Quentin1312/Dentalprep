...
export default function StatsPage() {
  ...
  return (
    <div style={{
      ...
      {/* Heatmap 90 jours */}
      {sessions.length > 0 && (
        <ActivityHeatmap sessions={sessions} today={today} />
      )}
...
function ActivityHeatmap({ sessions, today }: { sessions: Session[]; today: Date }) {
  ...
  function colorFor(min: number, isToday: boolean): string {
    if (isToday && min === 0) return '#FFFFFF'
    if (min === 0)  return '#EEF1F5'
    if (min < 15)   return '#BFD5F5'
    if (min < 30)   return '#7FAEEC'
    if (min < 60)   return '#3F87E0'
    return '#0A66E0'
  }

  const totalMinutes = cells.reduce((s, c) => s + c.min, 0)
  const activeDays = cells.filter(c => c.min > 0).length

  return (
    <div style={{ padding: '14px 16px 0' }}>
      <SectionLabel right={`${activeDays} jours actifs`}>Activité · 90 jours</SectionLabel>
      <div style={{
        background: '#fff', border: `1px solid #E4E8EE`, borderRadius: 18,
        padding: '14px 14px 12px',
        boxShadow: '0 1px 2px rgba(15,27,45,0.04), 0 6px 16px -10px rgba(15,27,45,0.15)',
          ...
          {columns.map((col, ci) => (
            <div key={ci} style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
              {Array.from({ length: 7 }).map((_, wd) => {
                const cell = col.find(c => c.weekday === wd)
                if (!cell) return <div key={wd} style={{ width: 11, height: 11 }} />
                return (
                  <div
                    key={wd}
                    ...
                    style={{
                      ...
                      background: colorFor(cell.min, cell.isToday),
                      border: cell.isToday ? '1.5px solid #0A66E0' : 'none',
                      boxShadow: cell.min > 0 ? 'inset 0 0 0 0.5px rgba(0,0,0,0.04)' : 'none',
                    }}
                  />
                )
              })}
        ...
        <div style={{
          ...
          <div style={{ fontSize: 10.5, color: '#5A6675', fontWeight: 700 }}>
            {totalMinutes} min sur 90 j
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 10, color: '#5A6675', fontWeight: 700 }}>
            Moins
            {['#EEF1F5', '#BFD5F5', '#7FAEEC', '#3F87E0', '#0A66E0'].map((c, i) => (
